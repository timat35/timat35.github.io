
<?php  include('title/title_prestation.php');?>
<?php  include('header.php');?>


<div  class ="container">
	<div class="col-12 text-center">
		<h2 class = "page_title">Prestations</h2>
	</div>
	<div class = "row">
		<div class = "col-8">
			<div class = "info-rect pad40">
				<p>Une peinture de chevalet est un objet complexe constitué de plusieurs strates qui présentent chacune une sensibilité particulière et seront traitées différemment.
				</p>
				<p>
				La première intervention  consiste à dresser un constat d’état pour recenser l’ensemble des altérations et déterminer les interventions prioritaires à mener.
				</p>
				<p>
				Le constat d’état est l’examen minutieux de l’œuvre et des matériaux qui la constituent.</br>Ces composants, le support, la couche picturale et le vernis sont soumis à un  vieillissement naturel qui conjugué à d’autres facteurs, comme les conditions environnementales, les accidents, les manipulations, accentuent et contribuent à sa dégradation.  
				</p>
				<p>
				C’est cet examen attentif et avec différents moyens, du support , des  vernis, de la couche colorée et de la préparation qui permet de diagnostiquer les altérations, d’en mesurer l’importance et d’établir un protocole de restauration déontologiquement conforme et adapté à l’œuvre confiée.
				</p>
				<p>
				Ces différentes étapes permettent d’établir un devis précis et détaillé des opérations de conservation et/ou  de restauration à envisager.
				</p>
			</div>
			<div class="pad40 mt-4">
				<h4>SERVICES PROPOSES GRATUITS ET SANS ENGAGEMENT</h4>
				<ul class="liste">
				<li><b>DEVIS</b></li>
				<li><b>DEPLACEMENT A DOMICILE</b></li>
				<li><b>CONSEILS EN CONSERVATION PREVENTIVE</b></li>
				</ul>
				<h4>DIFFERENTES ETAPES D’UNE RESTAURATION</h4>
				<ul class="liste">
				<li><b>DEVIS</b> établi à partir de l’examen approfondi de l’œuvre</li>
				<li><b>VALIDATION</b> du devis par le client</li>
				<li><b>REALISATION DE LA RESTAURATION</b> en suivant le protocole établi</li>
				<li><b>RESTITUTION</b> du bien confié en atelier ou à domicile</li>
				</ul>
			</div>
		</div>
		<div class = "col-3">
			<div class = "mt-3" style="text-align: center">
				<figure ind="0">
						<img src="img/prestation/prestation1.png" alt="tet-en-lair acceuil"/>
				</figure>
				<p class= "text-center">Stade ultime des altérations d’adhésion et de cohésion; perte de matière; perte de lisibilité; sans intervention l’œuvre est en péril</p>
			</div>
			<div class = "mt-3" style="text-align: center">
				<figure ind="1">
						<img src="img/prestation/prestation2.png" alt="tet-en-lair acceuil"/>
				</figure>
				<p class= "text-center">Jaunissement du vernis. L’allègement des vernis redonne de la luminosité et permet de retrouver les tonalités de la peinture originale</p>
			</div>
		</div>
	</div>
</div>



<?php  include('footer.php');?>

<script type="text/javascript" src="https://cdnjs.cloudflare.com/ajax/libs/jquery/2.1.3/jquery.min.js"></script>
<script type="text/javascript" src="js/jquery.easing.1.3.js"></script>
<script type="text/javascript" src="js/jquery.mousewheel-3.0.6.pack.js"></script>
<script type="text/javascript" src="js/main.js?v=1.5"></script>
<script type="text/javascript" src="js/imagesloaded.pkgd.min.js"></script>
<script type="text/javascript" src="js/photoswipe.js"></script>
<script type="text/javascript" src="js/photoswipe-ui-default.js"></script>
<script type="text/javascript" src="bootstrap/js/bootstrap.min.js"></script>



</body>
</html>


